...
const TestSystem = {
    ...
    initTest(chapter, questions) {
        ...
        function renderQuestions() {
            ...
            if (existingScore) {
                return `
                    <div class="glass-card rounded-3xl p-8 text-center">
                        ...
                        <p class="text-gray-400 mb-6">${existingScore.score} von ${existingScore.total} richtig</p>
                        <button class="glass-button-primary px-6 py-3 rounded-xl" onclick="TestSystem._resetKid(${currentKid})">Test wiederholen</button>
                    </div>
                `;
            }
            ...
            return questions.map((q, qi) => `
                <div class="question-card" id="q${qi}">
                    <p class="font-semibold mb-3 text-lg">${qi + 1}. ${q.question}</p>
                    <div class="space-y-2">
            ...
            `).join('') + `
                <div class="text-center mt-8">
                    ...
                    <p class="text-gray-500 text-sm mt-2" id="answer-count">0 von ${questions.length} Fragen beantwortet</p>
                </div>
            `;
        }
        ...
        TestSystem._answer = function(qi, oi, correct) {
            ...
            if (explanation && questions[qi].explanation) {
                explanation.textContent = questions[qi].explanation;
                explanation.classList.remove('hidden');
            }

            const count = Object.keys(answered).length;
            const countEl = document.getElementById('answer-count');
            if (countEl) countEl.textContent = `${count} von ${questions.length} Fragen beantwortet`;
